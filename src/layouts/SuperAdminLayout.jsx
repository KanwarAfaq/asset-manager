...
import { LayoutDashboard, Receipt, PieChart, Users, Tags, Settings as SettingsIcon, LogOut, Menu, Bell, Search, X } from 'lucide-react'
...
export default function SuperAdminLayout() {
  ...
  useEffect(() => {
    ...
    handleResize() // Pehli dafa check karein
  ...
  // Mobile par kisi link par click karne se sidebar band karne ka function
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans overflow-hidden">
      ...
      {/* SIDEBAR */}
      <aside className={`fixed md:relative inset-y-0 left-0 bg-white border-r border-gray-200 transition-all duration-300 ease-in-out z-50 flex flex-col h-screen
      ...
      `}>
        <div className="h-16 flex items-center justify-between px-6 border-b border-gray-100 shrink-0">
          ...
          {/* Desktop par toggle button */}
          <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="hidden md:block text-gray-500 hover:text-gray-800">
            <Menu size={24} />
          </button>
        </div>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link 
                ...
                className={`flex items-center gap-3 px-3 py-3 rounded-xl font-medium transition-all duration-200 ${isActive ? 'bg-blue-600 text-white shadow-md shadow-blue-200' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'}`}
                title={!isSidebarOpen ? item.name : ''}
              >
                <item.icon size={20} className={isActive ? 'text-white' : 'text-gray-400'} />
                <span className={`${!isSidebarOpen && 'hidden md:block whitespace-nowrap'}`}>{item.name}</span>
              </Link>
            )
          })}
        </nav>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden w-full">
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-gray-200 flex items-center justify-between px-4 md:px-8 z-10 shrink-0">
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(true)} className="p-2 rounded-lg text-gray-500 hover:bg-gray-100 transition-colors md:hidden">
              <Menu size={24} />
            </button>
